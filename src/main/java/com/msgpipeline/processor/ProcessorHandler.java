...
/**
 * =========================================================================
 * CLASE: ProcessorHandler — Lambda Entry Point (Sesión 08) (Handler de AWS Lambda)
 ...
 * ║    Lambda Runtime → Class.forName("ProcessorHandler") → new ProcessorHandler()  ║
 * ║    → handleRequest(SQSEvent, Context)                                ║
 * ║                                                                      ║
 * ║  AWS Lambda:                                                         ║
 * ║    1. NO ejecuta ningún main()                                       ║
 * ║    2. Instancia el handler via reflexión (new ProcessorHandler())          ║
 ...
 *   ProcessorHandler adapta el contrato de AWS Lambda (RequestHandler<SQSEvent, Void>)
 *   al contrato de negocio (ProcessMessagePort).
 *   Lambda solo sabe de SQSEvent. El negocio solo sabe de Message.
 *   ProcessorHandler hace la conversión entre ambos mundos.
 *
 ...
 */
@Slf4j
public class ProcessorHandler implements RequestHandler<SQSEvent, Void> {

    // ── Bloque static — Inicialización en el Cold Start ──────────────────
    //
    // COLD START: primera vez que Lambda ejecuta esta función.
    //   - La JVM se inicia
    //   - Lambda crea UNA instancia de ProcessorHandler (llama al constructor)
    ...
    static {
        log.info("╔═══════════════════════════════════════════════════════╗");
        log.info("║  ProcessorHandler — Inicialización Cold Start (Sesión 08)  ║");
        log.info("╚═══════════════════════════════════════════════════════╝");
